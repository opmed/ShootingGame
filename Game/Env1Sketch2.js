...
function preload() {
  ...
  img2 = loadImage('images/rifle.png'); //loading image
...
function setup() {
	createCanvas(windowWidth, windowHeight); //makes it fullsize
...
function draw() {
  ...
  noStroke(); //makes it without outline
  fill(0,0,0,0); //makes it black and transparent
  ellipse(820, 470, 120, 170); //setting elipse size and location
  pop(); //keeps it contained
  ...
  image(img2,690,550); //sets image location
...
let char1 = dist(mouseX, mouseY, 495, 530); //sets char1 to the distance around the image
  if (char1 < 80) { //sets distance to less than 80
   background(0); //sets background to black
   showing=false; //image disappears everytime mouse is clicked
   kills++ //increazes kills by 1
  }

let char2 = dist(mouseX, mouseY, 820, 470); //sets char2 to the distance around the image
  if (char2 < 80) { //sets distance to less than 80
   background(0); //sets background to black
